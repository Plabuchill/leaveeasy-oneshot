  ...
  if (window.auth) {
    window.auth.onAuthStateChanged(function (user) {
      ...
      if (!user) {
        ...
        // ล็อกอินอยู่แล้ว → ไม่ต้องเห็นหน้า login.html อีก (เด้งไปหน้าแรกแทน)
        // ⚠️ ไม่เด้งออกจาก signup.html ด้วยเหตุผลสำคัญ: signUp() เพิ่งสร้างบัญชีเสร็จ
        // แล้วยังต้องเขียนไฟล์ users/{uid} ลง Firestore ต่ออีกขั้นหนึ่ง (ดูฟังก์ชัน signUp
        // ด้านล่าง) ถ้าปล่อยให้ตรงนี้เด้งไปหน้า index.html ทันทีที่รู้ว่าล็อกอินแล้ว
        // จะไปแย่งจังหวะกับการเขียนไฟล์นั้น ทำให้คำขอเขียนถูกยกเลิกกลางคันและ
        // Firestore ตอบกลับมาเป็น permission-denied (ทดสอบแล้วเจอปัญหานี้จริง)
        // signUp() จัดการเด้งหน้าเองอยู่แล้วหลังเขียนไฟล์เสร็จ จึงไม่ต้องเด้งซ้ำที่นี่
